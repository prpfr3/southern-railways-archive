<?php
/**
 * Plugin Name: my-photo-importer-acf
...
add_action('admin_menu', function () {
    add_submenu_page(
        ...
        'Import Photo Products', // Page title
        'Import Photo Products', // Menu title
        'manage_options', // Capability
        'myplugin-import-data', // Menu slug
        'myplugin_import_page' // Callback function
    );
});
...
function myplugin_import_page() {
    ?>
    <div class="wrap">
        <h1>Import Photo Products</h1>
        Click the button below to start the import process.
        
        <form method="post">
            <input type="submit" name="myplugin_import_start" class="button button-primary" value="Run Import">
        </form>

        <?php
        // If the form is submitted, run the import function
        if (isset($_POST['myplugin_import_start'])) {
            myplugin_run_import();
        }
        ?>
    </div>
...
function myplugin_run_import() {
    ...
    echo "\nImport Running";
    $file = 'D:\OneDrive\Documents\Bluebell Railway\Archive Photodatabase\BRMPhotos.csv';
    $file = __DIR__ . '/BRMPhotos.csv';
    $row = 0;
    $created = 0;
    $skipped = 0;
    ...
    while (($row_data = fgetcsv($handle, 0, '|')) !== false) {
      ...
      echo "\n Deleting Product id: {$existing_product_id}";   
        ...
        $photos_category = get_term_by('name', 'Photos', 'product_cat');
        if ($photos_category) {
          wp_set_object_terms($product_id, [$photos_category->term_id], 'product_cat');
        } else {
          echo "\n⚠️ Category 'Photos' not found";
        }
        ...
        // External image
        $image_url = "https://www.bluebell-railway-museum.co.uk/archive/photos2/" .
        ...
        // Photographer taxonomy handling
        if (!empty($photographer)) {
          $photographer = sanitize_text_field($photographer);
          $term = term_exists($photographer, 'photographer');
          if (!$term) {
            $term = wp_insert_term($photographer, 'photographer');
          }
          if (!is_wp_error($term)) {
            $term_id = is_array($term) ? $term['term_id'] : $term;
            wp_set_object_terms($product_id, intval($term_id), 'photographer', false);
          } else {
            echo "\n❌ Photographer term error: " . $term->get_error_message() . "";
          }
        }
